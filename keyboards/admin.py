...
def player_card(player_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="👑 Роли staff", callback_data=f"admin:roles:{player_id}")],
...
def role_actions(player_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🛡 Модератор", callback_data=f"admin:set_role:moderator:{player_id}")],
        [InlineKeyboardButton(text="⚙ Админ", callback_data=f"admin:set_role:admin:{player_id}")],
        [InlineKeyboardButton(text="👑 Владелец", callback_data=f"admin:set_role:owner:{player_id}")],
        [InlineKeyboardButton(text="🚫 Снять роль", callback_data=f"admin:set_role:none:{player_id}")],
        [InlineKeyboardButton(text="⬅️ Назад", callback_data=f"admin:card:{player_id}")],
    ])
