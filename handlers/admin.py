...
from keyboards.admin import admin_panel, money_actions, player_card, role_actions
from utils.database import Database
from utils.logger import log_action
from utils.permissions import is_admin, is_owner, is_staff_group, is_staff_member
...
def with_staff_role(database: Database, player: dict) -> dict:
    admins = database.read("admins")
    player = dict(player)
    telegram_id = player["telegram_id"]
    if telegram_id in admins.get("owners", []):
        player["staff_role"] = "владелец"
    elif telegram_id in admins.get("admins", []) or telegram_id in admins.get("ids", []):
        player["staff_role"] = "админ"
    elif telegram_id in admins.get("moderators", []):
        player["staff_role"] = "модератор"
    else:
        player["staff_role"] = "нет"
    return player


@router.message(Command("panel"))
async def panel(message: Message, bot: Bot, database: Database, config: Config) -> None:
    if not is_staff_group(message, config) or not is_staff_member(message.from_user, database) or not message.from_user:
        return
...
async def panel_callback(callback: CallbackQuery, bot: Bot, database: Database) -> None:
    ...
    if not is_staff_member(callback.from_user, database):
        return
...
async def admin_search_start(callback: CallbackQuery, bot: Bot, database: Database, state: FSMContext) -> None:
    ...
    if not is_staff_member(callback.from_user, database):
        return
    if callback.data == "admin:roles" and not is_owner(callback.from_user, database):
        await replace_menu(bot, database, callback.message.chat.id, callback.from_user.id, "👑 Роли может выдавать только владелец.", admin_panel())
        return
    mode = callback.data
...
async def admin_search(message: Message, bot: Bot, database: Database, state: FSMContext) -> None:
    if not message.from_user or not is_staff_member(message.from_user, database) or not message.text:
        return
    ...
    if mode in {"admin:licenses", "admin:inventory"}:
        ...
        return
    if mode == "admin:roles":
        if not is_owner(message.from_user, database):
            await state.clear()
            await replace_menu(bot, database, message.chat.id, message.from_user.id, "👑 Роли может выдавать только владелец.", admin_panel())
            return
        await state.clear()
        await replace_menu(bot, database, message.chat.id, message.from_user.id, player_text(player), role_actions(player["telegram_id"]))
        return
    await state.clear()
    await replace_menu(bot, database, message.chat.id, message.from_user.id, player_text(with_staff_role(database, player)), player_card(player["telegram_id"]))
...
async def open_player_card(callback: CallbackQuery, bot: Bot, database: Database) -> None:
    ...
    if not player:
        ...
        return
    await replace_menu(bot, database, callback.message.chat.id, callback.from_user.id, player_text(with_staff_role(database, player)), player_card(target_id))
...
async def roles_menu(callback: CallbackQuery, bot: Bot, database: Database) -> None:
    await answer_callback(callback)
    if not is_owner(callback.from_user, database):
        await replace_menu(bot, database, callback.message.chat.id, callback.from_user.id, "👑 Роли может выдавать только владелец.", admin_panel())
        return
    target_id = int(callback.data.split(":")[-1])
    player = database.get_user(target_id)
    if not player:
        await replace_menu(bot, database, callback.message.chat.id, callback.from_user.id, "Игрок не найден.", admin_panel())
        return
    await replace_menu(bot, database, callback.message.chat.id, callback.from_user.id, player_text(with_staff_role(database, player)), role_actions(target_id))
...
async def set_role(callback: CallbackQuery, bot: Bot, database: Database) -> None:
    await answer_callback(callback)
    if not is_owner(callback.from_user, database):
        await replace_menu(bot, database, callback.message.chat.id, callback.from_user.id, "👑 Роли может выдавать только владелец.", admin_panel())
        return
    _, _, role, target_id_raw = callback.data.split(":")
    target_id = int(target_id_raw)
    admins = database.read("admins")
    for key in ("owners", "admins", "moderators", "ids"):
        admins.setdefault(key, [])
        admins[key] = [value for value in admins[key] if value != target_id]
    if role == "owner":
        admins["owners"].append(target_id)
        role_text = "владелец"
    elif role == "admin":
        admins["admins"].append(target_id)
        role_text = "админ"
    elif role == "moderator":
        admins["moderators"].append(target_id)
        role_text = "модератор"
    else:
        role_text = "нет"
    database.write("admins", admins)
    log_action(database, callback.from_user.id, "admin_set_role", target_id=target_id, role=role)
    player = database.get_user(target_id)
    if not player:
        await replace_menu(bot, database, callback.message.chat.id, callback.from_user.id, "Роль обновлена, но карточка игрока не найдена.", admin_panel())
        return
    await replace_menu(
        bot,
        database,
        ...
        f"✅ Роль обновлена: <b>{role_text}</b>\n\n{player_text(with_staff_role(database, player))}",
        role_actions(target_id),
    )
